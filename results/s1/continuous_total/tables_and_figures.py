...
def figures_and_tables_single():
	...
	plot_files = [split_plot_files,pivot_plot_files]
	top_labels = ["D$_0$", "D$_0$ - D$_1$", "D$_0$ - D$_2$", "D$_0$ - D$_3$", "D$_0$ - D$_4$"]
	plot_combined(plot_files, 'continuous_total_single.pdf', top_labels)
	...
	table_df.to_latex(open("PivotBi_single.tex", "w"), float_format="%.5f", caption="asd", label="tab:s1-cont-total-pivotbi-single", escape=False)
	for i in range(5):
	...
	table_df = pd.DataFrame(data, index=["F$_1$", "Jaccard", "Silhouette"], columns=top_labels)
	table_df.to_latex(open("SplitMerge_single.tex", "w"), float_format="%.5f", caption="asd", label="tab:s1-cont-total-splitmerge-single", escape=False)
...
def figures_and_tables_combined():
	...
	# TODO: 
	# When new S1 experiments are run, we need to align the clusters
	# over each segment. Print out the transitions while running?

	plot_files = [split_plot_files,pivot_plot_files]
	top_labels = ["D$_0$ - D$_1$", "D$_0$ - D$_2$", "D$_0$ - D$_3$", "D$_0$ - D$_4$"]
	plot_combined(plot_files, 'continuous_total_combined.pdf', top_labels)
	...
	data = [pivot_F, pivot_J, pivot_S]
	table_df = pd.DataFrame(data, index=["F$_1$", "Jaccard", "Silhouette"], columns=top_labels)
	table_df.to_latex(open("PivotBi_combined.tex", "w"), float_format="%.5f", caption="asd", label="tab:s1-cont-total-pivotbi-combined", escape=False)
	for i in range(4):
		df = split_plot_files[i]
		df = df.merge(basefile, on='_id', how='inner')
		split_F.append(cluster_wise_f_measure(df))
	...
	data = [split_F, split_J, split_S]
	table_df = pd.DataFrame(data, index=["F$_1$", "Jaccard", "Silhouette"], columns=top_labels)
	table_df.to_latex(open("SplitMerge_combined.tex", "w"), float_format="%.5f", caption="asd", label="tab:s1-cont-total-splitmerge-combined", escape=False)
