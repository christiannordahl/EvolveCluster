...
def figures_and_tables_single():
	...
	table_df = pd.DataFrame(data, index=["F$_1$", "Jaccard", "Silhouette"], columns=top_labels)
	table_df.to_latex(open("EvolveCluster_single.tex", "w"), float_format="%.5f", caption="asd", label="tab:s1-orig-eca-single", escape=False)
...
def figures_and_tables_combined():
	...
	data = [evolve_F, evolve_J, evolve_S]
	table_df = pd.DataFrame(data, index=["F$_1$", "Jaccard", "Silhouette"], columns=top_labels)
	table_df.to_latex(open("EvolveCluster_combined.tex", "w"), float_format="%.5f", caption="asd", label="tab:s1-orig-eca-combined", escape=False)
