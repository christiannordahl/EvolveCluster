...
def figures_and_tables_single():
	...
	plot_combined(plot_files, 'original_total_single.pdf', top_labels)
	...
	table_df.to_latex(open("PivotBi_single.tex", "w"), float_format="%.5f", caption="asd", label="asd", escape=False)
	for i in range(5):
	...
	table_df = pd.DataFrame(data, index=["F$_1$", "Jaccard", "Silhouette"], columns=top_labels)
	table_df.to_latex(open("SplitMerge_single.tex", "w"), float_format="%.5f", caption="asd", label="asd", escape=False)
...
def figures_and_tables_combined():
	...
	plot_combined(plot_files, 'original_total_combined.pdf', top_labels)
	...
	data = [pivot_F, pivot_J, pivot_S]
	table_df = pd.DataFrame(data, index=["F$_1$", "Jaccard", "Silhouette"], columns=top_labels)
	table_df.to_latex(open("PivotBi_combined.tex", "w"), float_format="%.5f", caption="asd", label="asd", escape=False)
	for i in range(4):
		df = split_plot_files[i]
		df = df.merge(basefile, on='_id', how='inner')
		split_F.append(cluster_wise_f_measure(df))
	...
	data = [split_F, split_J, split_S]
	table_df = pd.DataFrame(data, index=["F$_1$", "Jaccard", "Silhouette"], columns=top_labels)
	table_df.to_latex(open("SplitMerge_combined.tex", "w"), float_format="%.5f", caption="asd", label="asd", escape=False)
